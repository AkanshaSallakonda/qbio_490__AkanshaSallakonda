...
```{r}
write.csv(clinic, "/Users/akki/Desktop/USC/FreshmanYear/QBIO490/qbio_490_AkanshaSallakonda/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/akki/Desktop/USC/FreshmanYear/QBIO490/qbio_490_AkanshaSallakonda/analysis_data/brca_clinical_data.csv")
```
...
# 1. How many rows are there? How many columns? 112 columns, 1097 rows
...
print(nrow(clinic))
str(clinic)
patient_id_mask <- ifelse(clinic$Tumor_Sample_Barcode == 'TCGA-4H-AAAK', TRUE, FALSE)
clinic$age[patient_id_mask]
patient_age_last_mask <- ifelse(clinic$Tumor_Sample_Barcode == 'TCGA-3C-AAAU', TRUE, FALSE)
...
```{r Exercise 4.1}
# create plot here
plot(x=clinic$age_at_diagnosis, y=clinic$lymph_nodes_examined_count,
...
clinic$vital_status(vital_stats_na_mask)
```

2) Create a mask to select for male patients
```{r}
male_mask <- ifelse(clinic$gender == 'MALE', TRUE, FALSE)
clinic$bcr_patient_uuid[male_mask]
```

3) Create a mask to select for patients under the age of 35 OR over 60 at diagnosis. 
```{r}
under35_over60_mask <- ifelse(clinic$age_at_diagnosis < 35, 'young', ifelse(clinic$age_at_diagnosis >= 60, 'old', 'middle'))
clinic$age_category <- ifelse(clinic$age_at_diagnosis < 35, 'young', ifelse(clinic$age_at_diagnosis >= 60, 'old', 'middle'))
```

4) Using your under35_over60_mask, create a new dataframe called no_middle_clinic which contains all the clinical data for patients under the age of 35 or over 60 at diagnosis
...
cleaned_clinic <- clinic$age_at_diagnosis[age_NA_mask]
```
...
under35_over60_mask <- ifelse(clinic$age_at_diagnosis < 35, 'young', ifelse(clinic$age_at_diagnosis >= 50, 'old', 'middle'))
...
age_na_mask <- ifelse(clinic$death_days_to == "[Not Applicable]", F, T)
cleaned_clinic <- clinic[age_na_mask, ]

cleaned_clinic$survival_time <- ifelse(is.na(cleaned_clinic$death_days_to), cleaned_clinic$survival_time <- cleaned_clinic$last_contact_days_to, cleaned_clinic$survival_time <- cleaned_clinic$death_days_to)
...
#Middle, closely followed by old
...
#Hard to tell because x-axis is in the thousands?
...
#0.6, not statistically significant; we don't have evidence of a correlation between age and breast cancer
